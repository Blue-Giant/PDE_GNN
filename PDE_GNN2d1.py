...
def solve_multiScale_operator(R):
    ...
    Wout_trans = tf.get_variable(name='Wout_trans', shape=(out_dim2trans, out_dim2trans),
    ...
    with tf.device('/gpu:%s' % (R['gpuNo'])):
        with tf.variable_scope('vscope', reuse=tf.AUTO_REUSE):
            ...
            UNN = GNN_base1.HierarchicGKN(
                XY, coef_set=axy, PDE_type=R['PDE_type'], Ws2trans_input=wlist2trans_input,
                Bs2trans_input=blist2trans_input,
                actName2trans_input=actFunc2trans_input, hiddens2trans_input=hiddenlist2trans_input,
                scale_factor=freq_array, nn_idx=idx2knn, k_neighbors=knn2xy, Ws2kernel=wlist2kernel,
                Bs2kernel=blist2kernel, actName2Kernel=actFunc2kernel, hidden2kernel=hiddenlist2kernel,
                actName2GKN=actFunc2GKN, dim2linear_trans=out_dim2trans, W2linear_trans=Wlinear_trans,
                num2GKN=3, Wout=Wout_trans, Bout=Bout_trans, actName2out=actFunc2out)

            loss_u = tf.reduce_mean(tf.square(UNN - Utrue))
...
if __name__ == "__main__":
    ...
    R['hiddens2trans_input'] = (5, 10, 20, 30, 40)
    R['hiddens2kernel'] = (200, 250, 300, 350)
    R['out_dim2trans'] = 50
